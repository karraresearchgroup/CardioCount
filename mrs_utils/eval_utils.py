...
class ObjectScorer(object):
    ...
    def get_object_groups(self, conf_map):
        ...
        print('in mrs_utils, get_object_group, len of pixel groups > min_th and area > min_region = {}'.format(len(reg_props)))
        ...
        print('in mrs_utils, get_object_group, len of output {}'.format(len(pixel_grouped_reg_props)))
        #print(pixel_grouped_reg_props[0])
        return pixel_grouped_reg_props, pixel_grouped
...
def score(pred, lbl, min_region=5, min_th=0.5, dilation_size=5, link_r=20, eps=2, iou_th=0.5):
    ...
    group_pred, pixels_pred = obj_scorer.get_object_groups(pred)  # Ben added
    # print('doing object group for labels')        # Ben added
    group_lbl, pixel_lbl = obj_scorer. get_object_groups(lbl)
    # # Plotting the 
...
class Evaluator:
    def __init__(self, ds_name, data_dir, tsfm, device, load_func=None, infer=False, ensembler=None, **kwargs):
        ...
        #     self.class_names = ['panel', ]
            ...
            self.truth_val = kwargs.pop('truth_val', 255)       # Ben changed 2022.02.22 as trail 
