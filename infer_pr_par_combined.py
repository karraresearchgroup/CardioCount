...
def infer_confidence_map(DATA_DIR=DATA_DIR, SAVE_ROOT=SAVE_ROOT, FILE_LIST=FILE_LIST,
    ...
    """
    MODEL_maindir =  r'/home/home5/agk21/Karra_LVAD_NN/Collins_NeuralNets/ergmodels/'
    ...
    if extra_save_name is None:
        save_dir = os.path.join(super_args.save_root, os.path.basename(
            network_utils.unique_model_name(args)), "pred_masks")
    else:
    ...
    return os.path.basename(network_utils.unique_model_name(args)), exp_name, lossweight_name, classweight_name, training_val_lastiter
